...
if (command === "doctor") {
  ...
  console.error(
    "Usage: npm run stack -- <doctor|config|profiles|status|resources|smoke [--probe]|switch PROFILE [--dry-run|--require-ready|--skip-ready]|up PROFILE [--allow-gpu-share|--require-ready|--skip-ready]|build [SERVICE]|backend [ID...]|pull|logs [SERVICE]|stop [SERVICE]|down>"
  );
  process.exitCode = 2;
}
